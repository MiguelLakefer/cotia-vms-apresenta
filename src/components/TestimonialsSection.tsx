    ...
    <section id="testimonials" className="py-24 bg-gray-50">
      <div className="container">
        ...
        <div className="relative max-w-4xl mx-auto">
              ...
              {testimonials.map((testimonial) => (
                ...
                >
                  <div className="bg-white p-8 md:p-10 rounded-xl shadow-lg">
                    <Quote className="text-school-secondary h-10 w-10 mb-4 opacity-60" />
                    <p className="text-lg md:text-xl text-gray-700 mb-6 italic">
                      "{testimonial.content}"
                    </p>
                    <div>
                      <p className="font-bold text-lg">{testimonial.author}</p>
                      <p className="text-school-secondary">{testimonial.role}</p>
                    </div>
                  </div>
                </div>
              ))}
            ...
            {testimonials.map((_, index) => (
              <button
                ...
                className={cn(
                  ...
                  activeIndex === index ? "bg-school-secondary w-6" : "bg-gray-300"
                )}
          ...
          <button
            onClick={goToPrev}
            className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-2 md:-translate-x-6 bg-white rounded-full p-2 shadow-md hover:bg-gray-100 transition-colors"
          ...
          <button
            onClick={goToNext}
            className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-2 md:translate-x-6 bg-white rounded-full p-2 shadow-md hover:bg-gray-100 transition-colors"
