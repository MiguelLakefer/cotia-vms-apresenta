...
const TeacherCard = ({ name, role, image, description }: TeacherCardProps) => {
  return (
    <div className="group">
      <div className="overflow-hidden rounded-lg bg-white shadow-md transition-transform group-hover:-translate-y-2 group-hover:shadow-xl">
        <div className="relative h-64 overflow-hidden">
          ...
          />
          <div className="absolute inset-0 bg-gradient-to-t from-school-primary/80 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end p-6">
            <p className="text-white text-sm">{description}</p>
          </div>
        </div>
        <div className="p-6">
          <h3 className="text-xl font-bold mb-1 group-hover:text-school-secondary transition-colors">{name}</h3>
          <p className="text-school-secondary font-medium">{role}</p>
        </div>
      </div>
    </div>
  );
